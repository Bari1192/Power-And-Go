...
- **Főbb attribútumok**:
  - `protected $fillable`:
    - `plate`, `power_percent`, `power_kw`, `estimated_range`, `status`, `category_id`, `equipment_class`, `fleet_id`, `odometer`, `manufactured`.
...
- **Szabályok**:
  ...
  - `manufactured`: **Opcionális**, négy számjegyből álló **évszám**.
