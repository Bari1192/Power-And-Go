...
if ($kiterjesztes === 'json') {
    $autok_json_fajl_utvonal = $output_dir . '/Sulyozott_autok.json';
    file_put_contents($autok_json_fajl_utvonal, json_encode($autok, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
}
if ($kiterjesztes === 'csv') {
    $autok_csv_fajl_utvonal = $output_dir . '/Sulyozott_autok.csv';
    $fajl = fopen($autok_csv_fajl_utvonal, 'w');

    foreach ($autok as $auto_sor) {
        if (!empty($auto_sor)) {
            fputcsv($fajl, $auto_sor, ';');  // Használjuk az fputcsv-t, amely magától kezeli a sorvégeket
        }
    }
